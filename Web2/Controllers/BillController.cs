...
namespace Web2.Controllers
{
    ...
    public class BillController : ControllerBase
    {
        ...
        [HttpPatch("{id}/Update-Bill-Data")]
        [ProducesResponseType(typeof(BillDto), 200)]
        [ProducesResponseType(400)]
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<IActionResult> UpdateBillData(int id, [FromBody] BillDto dto)
        {
            try
            {
                if (id != dto.Id)
                ...
                var updated = await _billBusiness.Update(
                    dto.Id,
                    dto.Barcode,
                    dto.IssueDate,
                    dto.ExpirationDate,
            ...
            {
                _logger.LogWarning(ex, "Validación fallida al actualizar factura");
                return BadRequest(new { message = ex.Message });
            }
            catch (EntityNotFoundException ex)
            {
            ...
            {
                _logger.LogError(ex, "Error al actualizar factura con ID: {BillId}", id);
                return StatusCode(500, new { message = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> SetActiveStatus(int id, [FromBody] bool isActive)
        {
            try
            {
                var updatedNotification = await _billBusiness.SetActiveStatusAsync(id, isActive);
