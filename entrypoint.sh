...
run_migrations() {
    ...
    case "$DB_ENGINE" in
        sqlserver)
            ...
            echo "Aplicando migraciones de Entity Framework para SQL Server..."
            dotnet ef database update --context ApplicationDbContext --verbose || { echo "❌ ERROR: Falló la migración de SQL Server"; exit 1; }
            ;;
        postgres)
            ...
            echo "Aplicando migraciones de Entity Framework para PostgreSQL..."
            dotnet ef database update --context ApplicationDbContextPostgres --verbose || { echo "❌ ERROR: Falló la migración de PostgreSQL"; exit 1; }
            ;;
        mysql)
            ...
            echo "Aplicando migraciones de Entity Framework para MySQL..."
            dotnet ef database update --context ApplicationDbContextMySql --verbose || { echo "❌ ERROR: Falló la migración de MySQL"; exit 1; }
            ;;
        *)
    ...
    echo "✓ Base de datos y migraciones aplicadas exitosamente"
}
